...
class TopBar(tb.Frame):
    ...
    def show_notifications(self):
        ...
        try:
            ...
            tb.dialogs.Messagebox.show_error(
                f"Impossible de charger les notifications.\n\n{exc}",
                "Notifications",
            )
            return
        ...
        tb.Button(
            ...
            command=lambda: self._reload_notification_window(win),
            padding=(8, 5),
        ).pack(side="right")
    ...
    def _populate_notification_window(self, data):
        ...
        tb.Label(
            summary,
            text=f"🔴  {retard} facture(s) en retard",
            bootstyle="danger",
            font=("Segoe UI", 10, "bold"),
        ).grid(row=0, column=0, sticky="w", padx=(0, 10))
        tb.Label(
            summary,
            text=f"🟠  {echeance} échéance(s) sous 7 jours",
            bootstyle="warning",
            font=("Segoe UI", 10, "bold"),
        ...
        if total == 0:
            tb.Label(
                ...
                bootstyle="success",
                font=("Segoe UI", 11),
            ).pack(anchor="center", pady=35)
            return

        canvas = tk.Canvas(listing, highlightthickness=0, borderwidth=0, bg="#f4f6f8")
        ...
        if overdue:
            tb.Label(inner, text="🔴 Factures en retard", font=("Segoe UI", 11, "bold"), bootstyle="danger").pack(anchor="w", pady=(2, 7))
            for item in overdue[:20]:
                self._add_notification_row(inner, item, "danger")

        if due:
            tb.Label(inner, text="🟠 Prochaines échéances", font=("Segoe UI", 11, "bold"), bootstyle="warning").pack(anchor="w", pady=(14, 7))
            for item in due[:20]:
        ...
        if len(overdue) > 20 or len(due) > 20:
            tb.Label(
                inner,
                text="Affichage limité aux 20 notifications les plus pertinentes par catégorie.",
                bootstyle="secondary",
                wraplength=470,
    ...
    def _add_notification_row(parent, item, bootstyle):
        ...
        tb.Label(row, text=date_text, width=11, bootstyle=bootstyle).grid(row=0, column=0, sticky="w")
        tb.Label(row, text=f"{supplier}  •  {number}", font=("Segoe UI", 9, "bold"), anchor="w").grid(row=0, column=1, sticky="ew", padx=8)
        try:
            amount = f"{float(remaining or 0):,.2f} DA".replace(",", " ")
        except (TypeError, ValueError):
            amount = "0.00 DA"
        tb.Label(row, text=amount, bootstyle=bootstyle).grid(row=0, column=2, sticky="e")
