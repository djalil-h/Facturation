...
class Sidebar(tb.Frame):
    """Navigation principale Hamzaoui, plus large et lisible."""

    def __init__(self, master):
        super().__init__(master, width=242, bootstyle="dark")
        self.master = master
    ...
    def build(self):
        BrandLogo(self, width=222, height=122, dark=True, compact=True).pack(fill="x", padx=10, pady=(10, 6))
        tb.Separator(self, bootstyle="secondary").pack(fill="x", padx=14, pady=(0, 12))
        tb.Label(self, text="NAVIGATION", font=("Segoe UI", 10, "bold"), bootstyle="secondary-inverse").pack(anchor="w", padx=17, pady=(0, 7))
        ...
        self.logout_button = tb.Button(
            ...
            command=self.logout, padding=(10, 9), width=21
        )
    ...
    def add_button(self, key, text, command):
        btn = tb.Button(
            ...
            padding=(11, 10), width=23, compound="center",
        )
