...
from services.backup_service import create_import_backup
        ...
        for sheet in workbook.worksheets:
            ...
            for excel_row, row in enumerate(rows[header_index + 1:], start=header_index + 2):
                if not any(value not in (None, "") for value in row):
                    continue
                records.append({"sheet": sheet.title, "excel_row": excel_row, "mapping": mapping, "row": row, "supplier_hint": inferred_supplier})
        if not records:
...
def _record_to_data(record):
    ...
    return {"sheet": record["sheet"], "excel_row": record["excel_row"], "numero": numero, "fournisseur": supplier_name, "date_facture": date_facture, "date_echeance": date_echeance, "montant": montant, "montant_paye": montant_paye, "type_piece": type_piece, "commentaire": commentaire, "cheque": cheque, "date_paiement": _parse_date(_cell(row, mapping, "date_paiement", None), "Date paiement") if _cell(row, mapping, "date_paiement", None) not in (None, "") else None, "mode_paiement": str(_cell(row, mapping, "mode_paiement", "") or "").strip(), "reference_paiement": str(_cell(row, mapping, "reference_paiement", "") or "").strip()}
        ...
        for record in records:
            try:
                ...
                if key in existing_numbers:
                    data["status"] = "Doublon base"; data["action"] = "Ignoré"
                elif key in seen:
                    data["status"] = "Doublon Excel"; data["action"] = "Ignoré"
                else:
                    data["status"] = "OK"; data["action"] = "Import"; seen.add(key)
                rows.append(data)
            except Exception as exc:
                errors.append(f"Feuille {record['sheet']} — ligne {record['excel_row']} : {exc}")
        imports = [r for r in rows if r["action"] == "Import"]
        return {"filename": Path(filepath).name, "rows": rows, "errors": errors, "total": len(records), "valid": len(rows), "to_import": len(imports), "duplicates": len(rows) - len(imports), "avoirs": sum(1 for r in imports if r["type_piece"] == "Avoir"), "factures": sum(1 for r in imports if r["type_piece"] == "Facture"), "total_factures": sum(r["montant"] for r in imports if r["type_piece"] == "Facture"), "total_avoirs": sum(abs(r["montant"]) for r in imports if r["type_piece"] == "Avoir"), "suppliers": sorted({r["fournisseur"] for r in imports})}
    finally:
        session.close()
...
def importer_factures_excel(filepath, utilisateur=None, creer_fournisseurs=True):
    ...
    # Toujours créer un point de restauration avant une écriture en masse.
        ...
        for data in preview["rows"]:
            ...
            try:
                ...
                if fournisseur is None:
                    ...
                    fournisseur = Fournisseur(nom=data["fournisseur"]); session.add(fournisseur); session.flush(); supplier_cache[key] = fournisseur; suppliers_created += 1
                ...
                session.add(facture); session.flush()
                ...
                if montant_paye > 0:
                    ...
                    session.add(Paiement(facture_id=facture.id, montant=montant_paye, date_paiement=date_paiement, mode_paiement=data["mode_paiement"] or ("Chèque" if data["cheque"] else "Historique"), reference=data["reference_paiement"] or data["cheque"] or None, utilisateur_id=user_id)); payments_created += 1
                existing_numbers.add(data["numero"]); imported += 1
            except Exception as exc:
